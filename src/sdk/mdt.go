...
func CBuffer(str string) (*C.char, C.size_t) {
    var len C.size_t = C.size_t(len(str))
    var buf *C.char = (*C.char)(C.malloc(len))
    buf2 := (*[1<<30]C.char)(unsafe.Pointer(buf))
...
func Store(table *Table,
    ...
    // convert request
    c_request.primary_key.data, c_request.primary_key.size = CBuffer(primary_key)
    ...
    for i := C.size_t(0); i < c_request.index_list_len; i++ {
        c_index_list[i].index_name.data, c_index_list[i].index_name.size = CBuffer(index_list[i].IndexName)
        c_index_list[i].index_key.data, c_index_list[i].index_key.size = CBuffer(index_list[i].IndexKey)
    }
    c_request.data.data, c_request.data.size = CBuffer(data)
    
    // invoke C API
    C.mdt_store(table.rep, &c_request, &c_response, nil, nil)
    
    // convert result
    error := int32(c_response.error)
...
func SearchByIndexKey(table *Table,
    ...
    for i := C.size_t(0); i < c_request.index_condition_list_len; i++ {
        ...
        c_index_condition.index_name.data, c_index_condition.index_name.size = CBuffer(index_condition.IndexName)
        c_index_condition.comparator = index_condition.Comparator
        c_index_condition.compare_value.data, c_index_condition.compare_value.size = CBuffer(index_condition.CompareValue)
    }
